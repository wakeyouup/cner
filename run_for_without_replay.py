...
import time
import config_for_without_replay as config
...
def train(config):
    train_dir = os.path.join("save/life_long_without_er", config.experiment_type)
    ...
    for index, task_name in enumerate(task_sequence):
        ...
        if config.experiment_type == "upper_bound":
            ...
            print("在合并之前:", [len(v) for k, v in cur_dataset.items()])  # fixme: 我注释这里就是去除了er
        ...
        if config.experiment_type != "upper_bound" and index != len(task_sequence) -1 and config.use_examplar != False:
