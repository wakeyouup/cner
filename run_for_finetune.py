...
def train(config):
    train_dir = os.path.join("save/finetune", config.experiment_type)
    ...
    for index, task_name in enumerate(task_sequence):
        ...
        if config.experiment_type != "upper_bound" and index != len(task_sequence) -1 and config.use_examplar:
